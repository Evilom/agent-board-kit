...
**主电脑安装服务端＋客户端，副电脑只安装客户端。** 网络组件 `0.4.2` 提供设备与工程目录、跨设备文件和服务操作、已有配置引用及 5 个设备级 MCP 工具；本次修复服务分页 URL 和 Windows 进程测试清理。原会话入口提供 26 个工具，任务、消息、知识与本地 Board `1.1.1` 保持兼容。

## 跨设备工程与配置（0.4）

通过一个持久 MCP 入口找到已授权工程，读取上下文，直接调用目标设备的文件、固定命令和已配置服务。设备连接常驻，不依赖另一个聊天窗口在线，不在后台启动模型。MyWeb 等工程的知识和凭据继续使用原位置。

新入口：`network --config 本机配置.json ecosystem-connection`。完整登记、恢复、文件传输与凭据引用见[设备生态使用指南](docs/设备生态使用指南.md)。真实工程续接按[最后一公里验收](docs/最后一公里验收.md)执行；实际验证与未完成项见[0.4.2 验收记录](docs/升级验收记录-0.4.2.md)。
